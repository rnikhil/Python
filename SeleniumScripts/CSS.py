...
Every element does not have an id-­‐> static id, unique name,unique linktext.
For those elements we need to build xpath to find and then perform actions
on them.What ever we use to find an element,id,name,xpath,css-­‐>It should
always be unique.It should only find one matching node unless we want to
capture a list of elements.
Syntax:
tag[attribute='value']
“#” -­‐> Id
“.” -­‐> Class
...
“*” -­‐> Represents the text contained any where
Syntax:
...
Finding Children fieldset -­‐> 10 matching nodes
Fieldset>table
